...
## Evidence reviewed (updated 2026-07-31)
...
- 566 automated tests passed, 20 skipped. Additions since 2026-07-30 readiness: STS-123 append-only DPAPI-encrypted audit store with HMAC-SHA-256 per record, content-free (SHA-256 hashes only), purpose-limited (ADR-007); STS-103 AuthProvider interface with DevAuthStub default, WindowsCredentialLockerProvider, ADR-008 OS auth design, STRIDE threat model (STS-103-threat-model.md), 14 auth tests; STS-110 installer build automation — offline wheelhouse script, MSIX build layout, hash verification PowerShell scripts, AppxManifest.xml template, 19 installer tests; QG-04 pre-pentest hardening — HTTP security header audit (CSP no-wildcard, COOP, CORP, Permissions-Policy confirmed), path traversal analysis (SAFE — UUID paths, Path.name sanitization), SSRF analysis (N/A — localhost-only), pre-pentest hardening report, 11 new security header tests; QG-06 synthetic profiling — 5-scenario profiling protocol, mock-mode profiling script with VERBATIM_PERF_LIVE guard, dev-machine Docker simulation results (not_qualified_endpoint=true, SC-04 PENDING_HUMAN), 14 profiling tests; live mode wired (_live_process submits synthetic WAV to HTTP service, polls job status, records wall time and CPU/RSS). Additional 2026-07-31 additions: test_auth_credential_locker.py (15 mock-based ctypes tests, auth.py 49%→96%), test_audit_store_coverage.py (19 tests — Fernet encryption, retention_floor_reached, read_provenance HMAC, all 7 write methods, audit_store.py 82%→97%), overall coverage 87%→90%; evidence gate templates committed (evidence/os/endpoint-config-template.json QG-03, evidence/installer/clean-machine-matrix-template.json QG-02, evidence/accessibility/screen-reader-matrix-template.json QG-04, evidence/security/pentest-report-summary-template.json QG-03/QG-04). Additions prior to 2026-07-31: devcontainer policy (STS-112), Docker qualification image (STS-111), transcript correction/revision API (STS-102), accessibility contract matrix (STS-113), WER accuracy eval harness with 46 synthetic fixtures (STS-104), cross-environment evidence structure (STS-114), OWASP application-security acceptance tests (QG-04), DLP matrix, deletion drill guide, operator training and checklist (QG-05), synthetic media fixture stubs for all 8 supported formats, service/transcription/batch coverage gaps closed (service.py 67%→94%).
