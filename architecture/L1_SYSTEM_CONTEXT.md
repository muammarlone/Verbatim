...
Verbatim lets an authorized employee transcribe and review MP4 recordings on a managed Windows endpoint when cloud processing is unavailable or prohibited. The current product boundary is one operator, one operating-system account, one loopback process, local executables, local model files, and local storage.
...
|---|---|---|---|
| Authorized operator | Confirms authority, selects media/folders/formats, reviews source-linked text, exports, and deletes | MP4, relative folder choices, transcript review actions | Human authorization is required but not independently verified by the app |
...
Network deployment, shared accounts, speaker diarization, transcript editing, semantic or LLM summarization, live capture, records-system integration, automatic policy enforcement on exported copies, and compliance certification are not part of this architecture.
